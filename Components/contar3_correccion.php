...
FROM codigo WHERE codigo = $codigo ";
...
if ($result) {
  ...
  FROM contador WHERE item = $item and factura = '$factura' ";

  $result2 = $mysqli->query($query2);
  $num = mysqli_num_rows($result2);

  if ($num == 0) {
    echo $query9 = "INSERT INTO error_picking(cod,factura,usuario,item,fecha,hora)
    VALUES ('','$factura','$id',$item,'$fecha','$hora')";
    ...
    if ($result9) {
      ?>
        ...
        <span id="txt">
          <H1>
            EL PRODUCTO INGRESADO NO PERTENECE A ESTE DOCUMENTO DE VENTA
            <META HTTP-EQUIV='REFRESH' CONTENT='3;URL=contar_correccion.php'>
          </H1>
        </span>
        <?php
    }else{
...
if ($acumulador == $cantidad) {
  ...
  $query4 = "UPDATE contador SET contar = $acumulador, ok = 'Ok'
  WHERE item = $item and factura = '$factura'";

  $result4 = $mysqli->query($query4);

  if ($result4) {
    echo "<META HTTP-EQUIV='REFRESH' CONTENT='0;URL=contar_correccion.php'>";
  }else{
    echo "Error: ".$mysqli->error;
  }
  ...
  $query3 = "UPDATE contador SET contar = $acumulador, barras = '$codigo'
  WHERE item = $item and factura = '$factura'";
  ...
  $query5 = "INSERT INTO error_picking(cod,factura,usuario,item,fecha,hora)
  VALUES ('','$factura','$id',$item,'$fecha','$hora')";
...
?>
  ...
  <span id="txt">
    <H1>
      LA CANTIDAD CERTIFICADA ES MAYOR A LA SOLICITADA EN LA FACTURA
      <!-- <META HTTP-EQUIV='REFRESH' CONTENT='3;URL=contar_correccion.php'> -->
    </H1>
  </span>
  <?php
